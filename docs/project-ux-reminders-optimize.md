...
| ID | 內容 | 狀態 |
|----|------|------|
| A | 提醒依**項目**彙總（項目卡＋數量，展開才見事項） | 已完成 |
| B | 深連結 `/projects/{id}?task={taskId}`，詳情自動展開並捲動 | 已完成 |
| C | 預設「待處理」（已過期＋今天）；即將到期另篩 | 已完成 |
| D | 項目詳情頂部「本項目提醒」條 | 已完成 |
| L1 | 項目列表搜尋＋精簡卡片＋提醒角標 | 已完成 |
| L2 | 新增項目改為可收合／獨立區塊（＋開啟） | 已完成 |
| D1 | 詳情預設「待處理事項」視圖；結構（分類）為次要 | 已完成 |
| D2 | 手機 Tab：事項／帳本為主，「更多」收納其餘 | 已完成 |
